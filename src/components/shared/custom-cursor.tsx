...
// The brand pointer, inlined rather than fetched from /images so its fill can
// follow currentColor — the shape has to restate itself in black or white
...
// A black pointer disappears over the navy hero, the black footer and the
// navbar pill. Those surfaces carry data-cursor-tone="light".
...
// Dipped whenever the shape changes, then sprung back to 1, so the swap
// between pointer and caret bar lands as a beat rather than a cut.
...
function CustomCursor() {
  ...
  // Each shape offsets itself to its own hotspot, so the loop above only ever
  // deals with the raw pointer position. The pointer's tip is its top-right
  // corner; the caret bar is centred.
  return (
    <div
      ref={rootRef}
      aria-hidden="true"
